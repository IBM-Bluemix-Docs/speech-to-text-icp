...
You can also add grammars to a custom language model. Grammars restrict the service's response to only those words that are recognized by a grammar. For more information, see [Using grammars with custom language models](/docs/speech-to-text-icp?topic=speech-to-text-icp-grammars).
...
The new custom model is owned by the service instance whose credentials are used to create it. For more information, see [Ownership of custom models](/docs/speech-to-text-icp?topic=speech-to-text-icp-customization#customOwner).
...
-   For more information about using corpora, see [Working with corpora](/docs/speech-to-text-icp?topic=speech-to-text-icp-corporaWords#workingCorpora).
-   For more information about how the service adds corpora to a model, see [What happens when I add a corpus file?](/docs/speech-to-text-icp?topic=speech-to-text-icp-corporaWords#parseCorpus)
...
You can add a maximum of 90 thousand OOV words and 10 million total words from all sources combined. This includes words from corpora and grammars, and words that you add directly. For more information, see [How much data do I need?](/docs/speech-to-text-icp?topic=speech-to-text-icp-corporaWords#wordsResourceAmount)
{: note}
...
Use a loop to check the status of the corpus every 10 seconds until it becomes `analyzed`. For more information about checking the status of a model's corpora, see [Listing corpora for a custom language model](/docs/speech-to-text-icp?topic=speech-to-text-icp-manageCorpora#listCorpora).
...
-   For more information about adding words directly, see [Working with custom words](/docs/speech-to-text-icp?topic=speech-to-text-icp-corporaWords#workingWords).
-   For more information about how the service adds custom words to a model, see [What happens when I add or modify a custom word?](/docs/speech-to-text-icp?topic=speech-to-text-icp-corporaWords#parseWord)
...
As with adding corpora, examine the new custom words to check for typographical and other errors. This check is especially important when you add multiple words at once. For more information, see [Validating a words resource](/docs/speech-to-text-icp?topic=speech-to-text-icp-corporaWords#validateModel).
...
Use a loop to check the status every 10 seconds until it becomes `available`. For more information about checking the status of a custom model, see [Listing custom language models](/docs/speech-to-text-icp?topic=speech-to-text-icp-manageLanguageModels#listModels-language).
...
-   Use methods of the customization interface to examine the model's resources and fix any errors that you find:
    ...
    For more information about validating the words in a custom language model, see [Validating a words resource](/docs/speech-to-text-icp?topic=speech-to-text-icp-corporaWords#validateModel).
