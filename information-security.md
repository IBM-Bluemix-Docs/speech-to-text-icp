...
-   With batch-processing HTTP requests:
    -   `POST /v1/batches`

    The customer ID is associated with the data that is submitted with the batch-processing request.
...
"https://{icp_cluster_host}{:port}/speech-to-text/api/v1/user_data?customer_id=my_customer_ID"
```
{: pre}
